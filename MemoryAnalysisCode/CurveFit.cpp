...
//x and y_data should be set correctly befoe calling fit, result should be allocated 10+2 units
void fit(vector<unsigned> *x, long double *y_data, int num,vector<long double> &result)
{
    x_data = x;
    //unsigned X_NUM = num;
    ...
    for(j=0; j<FUNC_NUM; j++)
    {
        ...
        if(pre_best_squsum < EPSILON)// we have found best fit, no need calc rest function
            break;
    }
    return;
}
